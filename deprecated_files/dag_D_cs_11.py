...
my_dag = DAG(
    dag_id='D04_cosine-similarity_v11',
...
def cosine_similarity_A(top_n):
        ...
        for index in range(len(df)):
            ...
            dict_movie[index]= {'tconst_target': target_movie_id, 'tconst_reco':list_tconst, 'score_reco':list_score}
        ...
        #col_labels = ['tconst', 'tconst_reco', 'score_reco' ]
        df_score = pd.DataFrame.from_dict(data=dict_movie, orient='index')
        print(df_score.columns)

        # TEMP
        df_score.to_csv('/app/processed_data/extract_score.csv', index=False)

        # Store in MySQL DB
        df_score.to_sql('score_cs_c', engine, if_exists='replace', index=False)
