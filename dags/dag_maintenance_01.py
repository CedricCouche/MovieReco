...
import logging
import shutilfrom airflow import DAG
from airflow.providers.postgres.hooks.postgres import PostgresHook
from airflow.operators.python import PythonOperatorfrom dags import dag_makerdefault_args = {
    "owner": "airflow",
...
MAX_LOG_DAYS = 1
LOG_DIR = '/efs/airflow/logs/'def find_old_logs():
    ...
    conn = src_pg.get_conn()logging.info("Fetching old logs to purge...")with conn.cursor() as cursor:
        cursor.execute(sql)
        rows = cursor.fetchall()
        logging.info(f"Found {len(rows)} log directories to delete...")for row in rows:
            delete_log_dir(row[0])def delete_log_dir(log_dir):
    try:
        ...
        logging.info(f"Unable to delete: {e.filename} - {e.strerror}")with DAG(
